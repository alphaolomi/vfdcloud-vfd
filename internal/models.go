...
type (
	...
	// EFDMS is a struct that encapsulates the EFDMS data that is always
	// presented as XML.
	// Below is an example of the XML that is returned from the EFDMS API:
	//<?xml version="1.0"?>
	//<EFDMS>
	//	<RCT>
	...
	RCT struct {
		...
		TOTALS     []TOTAL    `xml:"TOTALS"`
		PAYMENTS   []PAYMENT  `xml:"PAYMENTS"`
		VATTOTALS  []VATTOTAL `xml:"VATTOTALS"`
	}
...
func ProduceEFDMS(efdms EFDMS) ([]byte, error) {
	output, err := xml.MarshalIndent(efdms, "", "	")
	if err != nil {
		return nil, err
	}
	return output, nil
